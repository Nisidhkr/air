...
/**
 * QR / code pairing for the device ecosystem ("verify both devices are
...
public final class QrPairingService {
    ...
    public PairingOffer start() {
        ...
        String payload = "air://pair/" + pairingId + "/" + secret
    ...
    public static Optional<Map<String, String>> parsePayload(String payload) {
        if (payload == null || !payload.startsWith("air://pair/")) {
            return Optional.empty();
        }
        try {
            String rest = payload.substring("air://pair/".length());
            int q = rest.indexOf('?');
